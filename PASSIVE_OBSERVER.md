...
> **Note**: Passive Observer is separate from the Kill Switch Protocol. The Kill Switch is always active and provides emergency stop capability regardless of Passive Observer status.
...
- **No silent isolation checks**: Agents can accidentally violate role separation unless they self-check
...
- **Edits to protocol docs aren't auto-flagged**:
  - Changes to `CLAUDE.md`, `protocol.config.yaml`, or agent files
...
- Check for forbidden terms in agent outputs
- Verify role separation maintained
- Confirm no inappropriate cross-agent references
...
- All `protocol/*.agent.md` files
- Any state files in `.protocol-state/`
...
#### 1. Proactive Safety
- **Automatic backup reminders**: Gojo nudges agents to create backups before changes
- **Isolation enforcement**: Silent checks that agents maintain role separation
...
- **Conflict resolution**: Arbitrates disagreements between agents
...
You can disable Passive Observer at any time in `protocol.config.yaml`:
...
| **Privacy** | No data collection | Session data collected |
...
- [ ] Agent outputs reviewed for role separation
- [ ] No inappropriate cross-agent references in dev-notes.md or security-review.md
...
**A**: No. The `.gitignore` file explicitly excludes `.protocol-state/trigger-19.md` by default. This file contains Gojo's private observations and is never committed.
...
**A**: Gojo clears temporary session data but preserves the Trigger 19 report until you manually delete it or it ages out per retention policy.

### Q: Does Passive Observer slow down my workflow?

**A**: No. Monitoring happens in the background and doesn't block agent work. Trigger 19 reports are generated on-demand, not continuously.

### Q: Can I customize what Gojo observes?

**A**: Yes. Edit `protocol.config.yaml` to adjust observation scope, data retention, and reporting frequency.

### Q: How does this relate to the Kill Switch?

**A**: They're independent. The Kill Switch (emergency stop) is **always active** regardless of Passive Observer status. Passive Observer is optional background monitoring.
...
- [`protocol/gojo.agent.md`](protocol/gojo.agent.md) - Gojo's full protocol
- [`protocol.config.yaml`](protocol.config.yaml) - Configuration reference
- [`docs/guides/EMERGENCY_STOP_GUIDE.md`](docs/guides/EMERGENCY_STOP_GUIDE.md) - Kill Switch guide
