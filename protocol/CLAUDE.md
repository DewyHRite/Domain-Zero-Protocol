...
- v9.10.1 - **PATCH**: Bundled remediation release closing the full 9-item queue deferred from the v9.10.0 Toji release-gate audit hold. Block A (distro integrity): new fail-closed `manifest_tracking_offenders()` gate verifying every manifest-shippable file is `git`-tracked after `git add -A`; closes `BUG-DISTRO-ORCH-TRIO-001` (stale v9.2.0 distro.gitignore block silently dropped the orchestration trio from published branches); trio now SHIPS (`ISS-084`/`ISS-085`). Block B (idgov polish): closes all 4 Toji-deferred findings — `DESIGN-001` resident-mint wrappers (`scripts/residentid-{sukuna,gojo,yuuji}.{sh,ps1}`), `IMPL-001`/`AI-001` report-contract validator (`scripts/check_toji_report_contract.py`), retro-mint exclusion audit (report-only). Block C (idgov registry lock hardening): two-reaper race fix (fresh in-lock re-validation + retryable `ConflictError`) + PID-recycle starvation backstop + `Lock.force_break()` break-glass; `engine.mint()` bounded retry + typed `DuplicateIdConflictError`. Block D (`brain.py` reset UX): `BUG-TEST-RESET-HANG-001` Part 2 product-side fix (`allow_prompt=False` unconditional on `--yes`). Plus `SEC-GUARD-007` (P1, CWE-345) stub-marker splice-bypass fix in `scripts/check_protected_append_only.py`. Megumi Tier-3 @approved twice (full bundle + focused verification). Toji pre-audit (`audits/2026-07-19-toji-v9-10-1-bundle-preaudit.md`) verified all 4 deferred findings RESOLVED and its own 4 new findings (3 MEDIUM, 1 LOW) closed same-release. Test evidence: 198/198 targeted remediation suites, `tests/test_check_toji_report_contract.py` 28/28, full sweep 2,376 passed / 4 skipped / 0 failed. Sukuna-implemented, Gojo-coordinated, USER-approved. See root `CLAUDE.md` and `CHANGELOG.md` `[9.10.1]` for the full changelog entry.
