...
**Complete override registry (v9.10.2)**: nine `DZP_ALLOW_*` overrides exist — `PROTECTED_REWRITE`
(append-only guard), `PROTOCOL_EDIT` (FEAT-REQ-001 stage only), `ISSUE_ID_OVERRIDE` (idgov
citation exceptions), `MISSING_ISSUE_ID_GATE` / `MISSING_APPEND_GUARD` / `MISSING_SECRET_SCAN`
(break-glass for absent gate engines — restore the gate instead of persisting these),
`TOJI_STUB_UNVERIFIED` (stub→report existence tripwire), `DIRTY_SOURCE` (`dzp-publish`
confirmed-clean-dirt only, rc==1), `UNHARDENED_IDGOV_TOKEN` (dev/test token-ACL relaxation).
All are loud (stderr, never silent) and must be scoped to a single invocation — never exported
globally or set in CI. Authoritative table: root `CLAUDE.md` § Override registry.
