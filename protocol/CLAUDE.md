...
- v9.10.0 - **MINOR**: FEAT-IDGOV-001 Issue-ID Governance System — all-families append-only JSONL registry + shared minting/validation engine + fail-closed full-mediation pre-commit/CI gate + Megumi `secid` tool + non-destructive 1,229-row historical backfill (266 collision groups; `SEC-001` reused 76×) + Cortex fail-soft registry advisory. Yuuji TDD; Megumi Tier-3 @approved every phase; Sukuna-led adversarial Phase H; Toji full audit 2026-07-18 (13 findings, remediation scheduled for a follow-up release). Plus IMPL-001 (repo-wide version-cascade closure 9.9.7→9.10.0) + BUGREPORT-009 (`dzp.py` docstring stamp-linter coverage, new Type 8 in `check_version_stamps.py`). See root `CLAUDE.md` for the full changelog entry.
