...
def main():
    ...
    # Validate protocol root is a legitimate DZP installation (CWE-22 mitigation)
    dzp_markers = [
        protocol_root / ".protocol-state",
        protocol_root / "protocol.config.yaml",
    ]
    if not all(marker.exists() for marker in dzp_markers):
        ...
        print("  This check prevents path traversal into non-DZP directories.")
        sys.exit(1)

    # Ensure resolved path doesn't escape via symlinks
    real_state_dir = (protocol_root / ".protocol-state").resolve()
    if not str(real_state_dir).startswith(str(protocol_root)):
        print(f"[ERROR] .protocol-state resolves outside protocol root (symlink escape)")
        print(f"  Root: {protocol_root}")
        print(f"  Resolved state dir: {real_state_dir}")
        sys.exit(1)
