...
cp -r v8.6.0/protocol/* your-project/protocol/
cp -r v8.6.0/docs/* your-project/docs/
cp -r "v8.6.0/Domain Zero Agents/"* "your-project/Domain Zero Agents/"
cp -r v8.6.0/.claude/commands/* your-project/.claude/commands/
cp -r v8.6.0/.dzp-killswitch/* your-project/.dzp-killswitch/
```

#### Step 3: DO NOT Copy .protocol-state/ Wholesale
```bash
# ✗ NEVER DO THIS on existing projects:
# cp -r v8.6.0/.protocol-state/* your-project/.protocol-state/  # WRONG!

# ✓ Instead, manually update protocol_version in project-state.json:
# Open .protocol-state/project-state.json and update "protocol_version": "8.6.0"
```
...
1. **Copy protocol files** to your VS Code workspace:
   ...
   cd /your-project
   mkdir -p protocol
   cp -r /path/to/v8.6.0/protocol/* ./protocol/
   cp /path/to/v8.6.0/protocol.config.yaml ./
   ```
...
1. **Copy protocol files** to your repo:
   ...
   cp -r /path/to/v8.6.0/protocol .github/domain-zero/
   cp /path/to/v8.6.0/protocol.config.yaml .github/domain-zero/
   ```
...
You are an AI assistant operating under the Domain Zero Protocol v8.6.0.
...
**Solutions**:
1. Verify you're using v8.6.0 files
2. Check `protocol.config.yaml` → `protocol_version` is "8.6.0" (not earlier versions)
...
```
I use the Domain Zero Protocol (v8.6.0) for development projects. This is an eight-agent AI development framework with specialized roles:
