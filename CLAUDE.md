...
> **Current Local Protocol Version**: v9.10.1
...
**Current Version**: 9.10.1
**Protocol Version**: 9.10.1
**Release Date**: 2026-07-19
**Last Updated**: 2026-07-19

**Recent Version History**:
- v9.10.1 - **PATCH**: Bundled remediation release closing the full 9-item queue deferred from the v9.10.0 Toji release-gate audit hold. **Block A** (distro integrity): new fail-closed, NO-override `manifest_tracking_offenders()`/`manifest_tracking_guard_cli()` gate in `scripts/distro/dzp_publish_core.py` verifying every manifest-shippable file staged under `distro/` is actually `git`-tracked after `git add -A`; closes `BUG-DISTRO-ORCH-TRIO-001` (a stale v9.2.0 "DEV-ONLY" `scripts/distro/distro.gitignore` block silently caused every published `DZP-vX.Y.Z` branch to drop the orchestration trio — `dzp.py`, `.protocol-state/script_coordinator.py`, `.protocol-state/script_dependencies.yaml` — even though the manifest-text and on-disk staging gates both reported green); USER ruling: the trio now SHIPS, fulfilling `ISS-084`/`ISS-085`. **Block B** (idgov polish, closes all 4 findings Toji deferred from the v9.10.0 audit): `DESIGN-001` RESOLVED via sanctioned `scripts/residentid-{sukuna,gojo,yuuji}.{sh,ps1}` resident-mint wrappers + `protocol/skills/resident-mint.md` (10-test parity suite `tests/test_residentid_wrapper.py`); `IMPL-001`/`AI-001` RESOLVED via new `scripts/check_toji_report_contract.py` mechanizing Toji's own report-contract requirements; report-only retro-mint exclusion audit `audits/2026-07-19-retromint-b1-exclusions.md`. **Block C** (idgov registry lock hardening, `scripts/idgov/registry.py`): C1 (P2) two-reaper race window closed via fresh in-lock re-validation (`_validate_fresh_before_write()` + retryable `ConflictError`); `engine.mint()` gains a bounded 3-attempt retry loop with typed `DuplicateIdConflictError` fail-fast classification (replacing a brittle message-substring check); C2 (P3) PID-recycle starvation backstop (`_STARVATION_TTL_MULTIPLIER` 10x-ttl) + human `Lock.force_break()` break-glass. **Block D** (`brain.py` reset UX): `BUG-TEST-RESET-HANG-001` product-side fix — `brain reset --yes`'s preserving-snapshot path now calls `_escrow_passphrase(allow_prompt=False)` unconditionally, so it can never block on a `getpass` prompt; complements the v9.10.0 test-harness-only fix as the genuine Part 2. **`SEC-GUARD-007`** (P1, CWE-345) — closes a stub-marker splice bypass in `scripts/check_protected_append_only.py`'s Toji-stub tripwire (a `[TOJI AUDIT LOG]` marker split across two no-trailing-newline commits could evade validation with zero warning) via a full-staged-content rescan with exact-line HEAD-identity exemption. Megumi Tier-3 @approved twice (full bundle, 2 rounds — 1 P1 + 4 P2 → @approved; plus a focused standalone verification of the report-contract validator, @approved). Toji's pre-audit (`audits/2026-07-19-toji-v9-10-1-bundle-preaudit.md`) independently re-verified all 4 v9.10.0-deferred findings (`SEC-001`, `DESIGN-001`, `IMPL-001`, `AI-001`) RESOLVED, and raised 4 new findings against the bundle itself (3 MEDIUM, 1 LOW) — all closed in this release: a dedicated `security-review.md` entry now records the `SEC-GUARD-007` review directly; the `BUG-TEST-RESET-HANG-001` two-part attribution is reconciled between `domain.record.md` and `brain.py`; the report-contract validator's unrecognized-field-label check is now implemented. Test evidence: 198/198 across the targeted remediation suites (0 regressions vs the pre-session 187/187 baseline), `tests/test_check_toji_report_contract.py` 28/28, full repo sweep **2,376 passed / 4 skipped / 0 failed**. Sukuna-implemented (System Update Adversary), Gojo-coordinated, USER-approved. See `CHANGELOG.md` `[9.10.1]` for the full itemized entry.
