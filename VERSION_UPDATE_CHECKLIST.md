...
**Current**: `Domain Zero Protocol v6.2.2`
**Action**: Rename folder to match new version (example: patch increment)
```bash
git mv "Domain Zero Protocol v6.2.2" "Domain Zero Protocol v6.2.2"
```
...
### Examples:
- `v6.2.0` → `v6.2.2`: Bug fix or documentation update
- `v6.1.0` → `v6.2.0`: New agent added, new features, enhanced workflow
- `v6.2.2` → `v7.0.0`: Breaking changes to protocol structure
...
if [ -z "$OLD_VERSION" ] || [ -z "$NEW_VERSION" ]; then
    echo "Usage: ./update-version.sh v6.2.0 v6.2.2"
    exit 1
fi
