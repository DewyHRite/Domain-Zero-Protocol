# FEAT-IDGOV-001 Yuuji-signed issue-id wrapper (residentid-yuuji.ps1) --
# PowerShell parity with scripts/residentid-yuuji.sh; see that file's header
# for the full contract (Toji DESIGN-001 remediation, v9.10.1 Block B2).
#
# The ONLY sanctioned path that stamps attested_writer=yuuji: computes a
# FRESH random nonce per invocation, signs it with Yuuji's own per-wrapper
...
# the caller's cwd, so `residentid-yuuji.ps1` behaves identically no matter
# where it is invoked from.
#
# See protocol/skills/resident-mint.md and
# docs/superpowers/specs/2026-07-13-issue-id-governance-design.md.
#
# Usage: scripts/residentid-yuuji.ps1 <check|list|new|state|validate> [args...]
#
...
$signOut = & python -c $SignScript $ScriptDir 'yuuji'
if ($LASTEXITCODE -ne 0) {
    ...
    [Console]::Error.WriteLine("residentid-yuuji: failed to sign request (Yuuji token provisioning/signing error)")
    exit 2
}

$lines = @($signOut) | Where-Object { $_ -ne "" }
if ($lines.Count -lt 2) {
    [Console]::Error.WriteLine("residentid-yuuji: malformed signing output")
    exit 2
}
