#!/usr/bin/env sh
# FEAT-IDGOV-001 Yuuji-signed issue-id wrapper (residentid-yuuji.sh)
#
# Parity wrapper for scripts/secid.sh (Toji DESIGN-001 remediation, v9.10.1
# Block B2) -- the ONLY sanctioned path that stamps attested_writer=yuuji on
# a mint/state transition. Computes a FRESH random nonce per invocation,
# signs it with Yuuji's own per-wrapper token (idgov.identity.sign), exports
...
#
# Yuuji is authorized (scripts/idgov/engine.py AUTHORITY) for: BUG, IMPL,
# TEST. Minting/transitioning any other family fails closed with a "not
# authorized" error from the engine -- this wrapper does not pre-filter, the
# engine is the single source of truth for the authority matrix.
#
...
# cwd), so `residentid-yuuji.sh` behaves identically no matter where it is
# invoked from.
#
# See protocol/skills/resident-mint.md and
# docs/superpowers/specs/2026-07-13-issue-id-governance-design.md.
#
# Usage: scripts/residentid-yuuji.sh <check|list|new|state|validate> [args...]
#
...
if ! dzp_python_probe; then
    echo "residentid-yuuji: no python3/python interpreter found on PATH" >&2
    exit 2
fi
...
token_id = identity.provision("yuuji")
nonce = secrets.token_hex(16)
sig = identity.sign(token_id, nonce)
print(nonce)
print(sig)
' "$SCRIPT_DIR") || {
    echo "residentid-yuuji: failed to sign request (Yuuji token provisioning/signing error)" >&2
    exit 2
}
