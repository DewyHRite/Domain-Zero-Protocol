...
$signOut = & python -c $SignScript $ScriptDir 'gojo'
if ($LASTEXITCODE -ne 0) {
    ...
    [Console]::Error.WriteLine("residentid-gojo: failed to sign request (Gojo token provisioning/signing error)")
    exit 2
}

$lines = @($signOut) | Where-Object { $_ -ne "" }
if ($lines.Count -lt 2) {
    [Console]::Error.WriteLine("residentid-gojo: malformed signing output")
    exit 2
}
